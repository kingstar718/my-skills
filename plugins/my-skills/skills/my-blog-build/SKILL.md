...
- **代码块**：必须标记语言 `` ```typescript ````，行内代码用单反引号
...
每篇内容在生成或修改后，必须逐项自检全部通过才能提交：

- **简洁**：一句话能说清的不用一段话。删掉冗余引导词。每个段落只承载一个核心信息点。
- **明了**：技术概念首次出现时用一句话解释，不假设读者已掌握。复杂流程用有序列表拆解，不堆砌成段落。
- **不说废话**：与主题无关的套话、客套、铺垫、总结性重复一律不写。开头直接进入内容，结尾不写空总结。
- **流畅**：段落之间逻辑衔接自然，避免跳跃。从「是什么」到「为什么」再到「怎么做」的顺序展开。
- **用词规范**：技术术语通篇统一，中文技术词汇参考业界通用译法（如「渲染」不写「绘制」、「构建」不写「编译」）。
- **可读性**：正文段落不超过 5 行（以 80 字符宽度计）。关键操作步骤用有序列表，配配置/代码用代码块。适当使用加粗强调关键结论。
- **复验**：生成后通读一遍，检查是否有主谓不搭配、时态混乱、语序倒错、重复啰嗦。技术描述与实际代码行为是否一致。列表项之间无交叉遗漏。

### 禁用表达清单（出现即不合格）

- **万能开头**：「在当今」「随着…的发展」「近年来」「众所周知」「随着互联网的普及」等
...
- **短文（notes）**：1-2 段成文，不设小节，直接说事
- **文章（posts）**：每节只围绕一个子主题；h2 标题是信息本身，不用「前言/正文/结语」这类骨架词
