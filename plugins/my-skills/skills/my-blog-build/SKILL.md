...
- **中文排版**：中英文之间加空格（标点规则、数字术语见 `cn-writing`）
- **文件名 = slug**：小写英文 + 连字符（`my-post.md`）；中文标题交给 `title`，不进文件名。新文章编辑器产出的 slug 形如 `2026-08-25-1752000000000.md`（UTC 日期-毫秒时间戳）

### 标题标签（末尾 `#标签`）

- **AI 生成或重写正文的文章，`title` 末尾空一格加 `#ai`**：`一台 2G ECS 的 pi-web 运维记录 #ai`；人自己写的文章不加
- **`title` 含 `#标签` 时 frontmatter 值必须加引号**：`title: "一台 2G ECS 的 pi-web 运维记录 #ai"`。不加引号时 YAML 把空格后的 `#` 当注释截断，标题标签会静默丢失（编辑器保存会自动加引号，脚本 PUT 裸值会踩这个坑）
- 可以有多个标签（`xx #ai #长文`），无标签页/标签表——标签只是说明，不是分类系统
- 只认**结尾处**的标签（`(?:\s+#[^\s#]+)+$`），正文标题里出现 `C# 入门` 这类不受影响
- frontmatter 里存完整标题字符串，渲染时才拆开（`src/lib/title.ts` + `EntryTitle.astro` 淡显标签）；复制标题时带空格原文，就是「标题 #ai」
- slug 不受影响，仍是纯标题的连字符形式
...
- [ ] AI 生成/重写的文章 `title` 末尾有 ` #ai`，人写的没有
- [ ] `updated` 已在修改时更新
- [ ] 图片引用用 `/media/...`，不是本地路径
